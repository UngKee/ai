...
function _limitSign(obj){
  const raw=obj.date+'|'+obj.count+'|'+_fpId+'|nekograf_v2'
  let h=0x9e3779b9
...
;(function(){
  ...
  const _limitPattern=/nekograf_limit/
...
async function sendCurrentText(){
  ...
  if(detectPromptInjection(a)){
    ...
    const _rjReplies=[
      ...
      'Saya mendeteksi upaya untuk mengganti identitas saya. Tidak akan berhasil — saya tetap Nekograf. Ada yang bisa saya bantu?',
...
;(function(){
  ...
  shareBtn.addEventListener('click',async function(){
    ...
    const title=(entry.topic)||'Obrolan Nekograf AI'
    const payload={
      id:currentChatId,
      topic:title,
      createdAt:Date.now(),
    ...
    if(navigator.share){
      try{await navigator.share({title:'Nekograf AI — '+title,url:shareUrl});return}catch(e){}
    }
    try{
...
(function(){
  ...
  ;(function(){
    const _monitoredKeys=new RegExp('nekograf_limit|nekograf_device|nklm_')
